...
- **Al Cargar (al enchufar):** Modo `0x05` (Steady Stream / Flujo) a velocidad mínima con **gradiente progresivo HSV** según nivel de batería (`≤15%` Rojo → Ámbar → Amarillo → Lima → `100%` Verde esmeralda, interpolado por tono).

### E. Estado de la Palanca / Switch y Opciones (Opcode `0x86`):
...
### F. Interfaz gRPC en Windows (`127.0.0.1:3814`):
...
### G. Matriz RGB Tecla por Tecla / Custom Canvas (Opcodes `0x07` y `0x0C`):
