...
| **Teclado Akko 5075B Plus** | USB HID `3151:4015` (USB) / `3151:4011` (2.4G), ROYUAN B-series, iface 2, Feature Report 65 B | `sync-rgb.py:sync_akko_keyboard`, `mchose-battery` | Retro de teclas (opcode `0x07`) + tira lateral (opcode `0x08`) | Backlight sólido sincronizado; Tira lateral reactiva: Steady Stream (`0x05`) con gradiente progresivo al cargar, Breathing rojo (`0x02`) con batería ≤ 20%, y color estático del tema en normal. Telemetría real de batería (opcode `0x83`) |
