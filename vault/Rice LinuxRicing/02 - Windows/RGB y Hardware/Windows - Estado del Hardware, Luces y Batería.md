...
- En modo **2.4G inalámbrico** se identifica como **`PID 0x4011`** (se consulta mediante gRPC con `dangledevtype=1` y Opcode `0x83`). El dongle USB en HID crudo responde con un descriptor estático Family B (`0x42`), por eso el canal bidireccional real pasa por el puente gRPC del Akko Cloud Driver (`127.0.0.1:3814`).
